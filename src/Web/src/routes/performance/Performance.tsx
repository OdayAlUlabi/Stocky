      ...
      {isLoading ? <Loader /> : !data ? <EmptyState title="No performance data yet" description="Snapshots accrue daily; come back tomorrow." /> : (
        <>
          <SimpleGrid cols={{ base: 2, md: 4 }}>
            <Card withBorder><Text size="xs" c="dimmed">TWR</Text><Text fw={600} c={data.twrPercent >= 0 ? 'teal' : 'red'}>{data.twrPercent.toFixed(2)}%</Text></Card>
            <Card withBorder><Text size="xs" c="dimmed">MWR (approx)</Text><Text fw={600} c={data.mwrPercent >= 0 ? 'teal' : 'red'}>{data.mwrPercent.toFixed(2)}%</Text></Card>
